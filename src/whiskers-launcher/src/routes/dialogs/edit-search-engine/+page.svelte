<script lang="ts">
	...
	onMount(async () => {
		...
		convertedIconPath = iconPath
			? `${convertFileSrc(iconPath!!)}?${Math.floor(Math.random() * 696969)}`
			: null;

		tintIcon = engine.tint_icon;
		keyword = engine.keyword;
		name = engine.name;
		searchQuery = engine.search_query;
...
{#if showPage}
	...
	<div class=" bg-background p-4 w-full h-screen overflow-auto text-text flex flex-col">
		...
		<div class=" mt-4 flex justify-end">
			<PrimaryButton text="Save" disabled={disableButton} on:click={save} />
		</div>
	</div>
{/if}
