...
pub fn cli_search(search_text: &str) {
...
fn show_results(results: &Vec<SearchResult>) {
    let search_results = SearchResults::new_list_results(results.clone());
    let search_results_json =
        serde_json::to_string(&search_results).expect("Error serializing search results");

    println!("{search_results_json}");
    exit(0);
}

fn get_recent_apps() -> Vec<App> {
    match fs::read(get_recent_apps_path()) {
        Ok(bytes) => bincode::deserialize(&bytes).expect("Error deserializing recent apps"),
        Err(_) => vec![],
    }
}
