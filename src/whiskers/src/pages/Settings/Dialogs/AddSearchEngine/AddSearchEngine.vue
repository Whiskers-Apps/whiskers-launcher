...
async function addSearchEngine() {
  ...
  emit("add-search-engine", payload);
...
<template>
  <div v-if="uiState.hasLoaded" class="main h-screen max-h-screen p-4 flex flex-col bg-red-400">
    <div class="flex-grow overflow-auto">
      <div class="flex justify-center">
        <div class="flex flex-col items-center">
          <button class="icon-box aspect-square rounded-2xl scale-100" @click="selectIcon()">
            <img
              v-if="uiState.iconPath !== null && !uiState.tintIcon"
            ...
            <img
              v-if="uiState.iconPath !== null && uiState.tintIcon"
              class="h-full w-full rounded-2xl scale-100"
              :src="uiState.iconPath"
              :style="{
          ...
          <SecondaryButton
            class="mt-2"
            v-if="uiState.realIconPath !== null"
            text="Clear Icon"
      ...
      <SwitchSetting
        title="Tint Icon"
        description="If enabled, it tints the search engine icon"
        :checked="uiState.tintIcon"
        :theme="uiState.settings!!.theme"
        @update-checked="uiState.tintIcon = $event"
      />
      ...
      <InputSetting
        ...
        :theme="uiState.settings!!.theme"
        @update-value="uiState.keyword = $event"
      />
      <InputSetting
        class="mt-2"
        title="Name"
        ...
        :value="uiState.name"
        :theme="uiState.settings!!.theme"
        @update-value="uiState.name = $event"
      />
      <InputSetting
        class="mt-2"
        title="Query"
        description="The search engine query. Tip: Use %s as the search text"
        placeholder="Query"
        :value="uiState.query"
        :theme="uiState.settings!!.theme"
